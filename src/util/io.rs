...
#[inline]
pub fn io_err_result<T, E: Into<BoxedError>>(
    result: Result<T, E>,
    kind: ErrorKind,
) -> io::Result<T> {
    match result {
        Ok(t) => Ok(t),
        Err(err) => Err(io::Error::new(kind, err)),
    }
}

/// Map a `None` optional into an `std::io::Error`
#[inline]
pub fn io_err_option<T, E: Into<BoxedError>, F: Fn() -> E>(
    option: Option<T>,
    kind: ErrorKind,
    err: F,
) -> io::Result<T> {
    match option {
        Some(t) => Ok(t),
        None => Err(io::Error::new(kind, err())),
    }
}
